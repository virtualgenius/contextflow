...
1. ~~**Business Model Role**~~: Implemented as `businessModelRole` dropdown in the Inspector with values: revenue-generator, engagement-creator, compliance-enforcer, cost-reduction. Syncs via Yjs, tracked in analytics.
...
| Ubiquitous Language | -- | Not supported yet (see gaps below) |
...
**Good fit for ContextFlow:**

1. **Domain Roles / Model Traits** (15 archetypes from Alberto Brandolini)
...
2. **Ubiquitous Language** (key terms and definitions per context)
...
3. **Assumptions** (explicit design uncertainties)
...
5. **Inbound/Outbound Communication** (commands, queries, events per collaborator)
