...
import { migrateLegacyPerSidePattern } from './migrateLegacyPerSidePattern'
...
describe('migrateLegacyPerSidePattern', () => {
  ...
  it('leaves separate-ways pattern unchanged (Slice 8 territory)', () => {
    const out = migrateLegacyPerSidePattern(rel({ pattern: 'separate-ways' }))
    expect(out.pattern).toBe('separate-ways')
  })
