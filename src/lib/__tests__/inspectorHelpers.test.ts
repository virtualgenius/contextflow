...
describe('getConnectedUsers', () => {
  ...
  it('returns users connected to a context via user needs', () => {
    const project = makeProject({
      ...
      userNeeds: [{ id: 'need-1', name: 'Search', position: 40 }],
      userNeedConnections: [{ id: 'uc1', userId: 'u1', userNeedId: 'need-1' }],
      needContextConnections: [{ id: 'nc1', userNeedId: 'need-1', contextId: 'ctx-1' }],
    })
  ...
  it('deduplicates users connected through multiple needs', () => {
    const project = makeProject({
      users: [{ id: 'u1', name: 'Alice', position: 50 }],
      userNeeds: [
        { id: 'need-1', name: 'Search', position: 40 },
        { id: 'need-2', name: 'Browse', position: 50 },
      ],
...
describe('categorizeRelationships', () => {
  ...
  it('categorizes shared-kernel as mutual', () => {
    const rels = [
      makeRel({ id: 'r1', fromContextId: 'ctx-1', toContextId: 'ctx-2', pattern: 'shared-kernel' }),
    ]
    const result = categorizeRelationships(rels, 'ctx-1')
  ...
  it('categorizes partnership as mutual', () => {
    const rels = [
      makeRel({ id: 'r1', fromContextId: 'ctx-2', toContextId: 'ctx-1', pattern: 'partnership' }),
    ]
    const result = categorizeRelationships(rels, 'ctx-1')
    expect(result.mutual).toHaveLength(1)
  })
